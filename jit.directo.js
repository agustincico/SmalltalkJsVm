 ...
 *
 * ============================ VEREDICTO MEDIDO ============================
 * ESTE ARCHIVO NO ESTÁ EN EL PRODUCTO Y NO DEBE ENCENDERSE. Ningún entry point
 * (squeak.js / squeak_headless.js / squeak_node.js) lo carga: vive sólo en
 * utils/arneses-node/ como artefacto de investigación.
 *
 * Es correcto: traza IDÉNTICA al clásico sobre 20.041.859 sends, verificada con
 * el oráculo diferencial. Y es 19,4x en sends de microbenchmark.
 *
 * Pero en el trabajo que importa es PEOR. Morphic real con display offscreen,
 * 20M sends (mismo trabajo), 3 pares intercalados, la máquina quieta:
 *     clásico  5398 ms  (mejor)   |   directo  6085 ms   =  12,7% MÁS LENTO
 * 3 de 3 pares a favor del clásico. La causa está medida y es estructural: los
 * métodos con bloques son inelegibles, Morphic está hecho de bloques, y cada
 * rotura de cadena cuesta más que el send clásico que reemplaza.
 *
 * Tres hipótesis de rescate, las tres MEDIDAS Y DESCARTADAS — no reabrir ninguna
 * sin datos nuevos:
 *   - inline cache POLIMÓRFICO: capturaría el 1,5% de los fallos (98,4% de los
 *     fallos son 16 sitios megamórficos, donde un PIC también se rinde);
 *   - soportar BLOQUES: con el veto apagado siguen siendo el 1,8% de las
 *     fronteras, y habilitar a los creadores no elimina ni una frontera de value:;
 *   - llamar PRIMITIVAS REALES desde código directo: implementado, correcto y
 *     trace-idéntico, cortaba el 64% de las roturas de cadena — y el reloj no se
 *     movió (tinyBenchmarks ±0,4%, Morphic 3% peor). Borrado en esta limpieza;
 *     está en el commit 43793d2 si alguna vez hace falta.
 * El patrón se repitió tres veces: el cuello aparente no era el cuello.
 * ==========================================================================
 *
