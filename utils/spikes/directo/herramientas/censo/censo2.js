...
fs.readFile(root + imageName + ".image", function(error, data) {
    ...
    image.readFromBuffer(data.buffer, function() {
        ...
        var saltosAtras = {jump:0, jumpTrue:0, jumpFalse:0}, saltosAdelante = {jump:0, jumpTrue:0, jumpFalse:0};
        var metodosConLoop = 0;
        ...
        vm.allMethodsDo(function(classObj, methodObj, selectorObj) {
            ...
            if (!quickPrim) {
                ...
                while (!done && pc < bytes.length) {
                    ...
                    if (/^jump/.test(ins.kind)) {
                        if (ins.dests[0] < ins.fin) saltosAtras[ins.kind]++; else saltosAdelante[ins.kind]++;
                    }
            ...
            var hayAtras = false;
            for (var pk in insns) { var ik = insns[pk]; if (/^jump/.test(ik.kind) && ik.dests[0] < ik.fin) { hayAtras = true; break; } }
            if (hayAtras) metodosConLoop++;
        ...
        console.log("saltos hacia atras: jump=" + saltosAtras.jump + " jumpTrue=" + saltosAtras.jumpTrue + " jumpFalse=" + saltosAtras.jumpFalse);
        console.log("saltos adelante:    jump=" + saltosAdelante.jump + " jumpTrue=" + saltosAdelante.jumpTrue + " jumpFalse=" + saltosAdelante.jumpFalse);
        console.log("metodos con al menos un salto hacia atras (loop): " + metodosConLoop);
        console.log("");
